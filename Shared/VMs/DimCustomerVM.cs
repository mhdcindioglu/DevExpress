namespace DevExpress.Shared.VMs;

public class DimCustomerVM
{
    public int CustomerKey { get; set; }
    public int? GeographyKey { get; set; }
    public required string CustomerAlternateKey { get; set; }
    public string? Title { get; set; }
    public string? FullName => $"{FirstName}{(string.IsNullOrEmpty(MiddleName) ? null : $" {MiddleName}")}{(string.IsNullOrEmpty(LastName) ? null : $" {LastName}")}";
